...
def fm_textbank(table, attr):
    ...
    while x:
        ...
        num = random.randint(1,attr.textbank_linecnt)        #gets random number from 0 to the line count
        i = 1                          #start index
        
        #cycle to find the right line
        for line in attr.textbank_fd:        
            if i == num:               #once we get to the wanted line
                value = line.rstrip()  #it contains the value we want (cuts the newline)
                break;        
    ...
    attr.textbank_fd.seek(0)          #for potential other attributes using the same textbank
    return "'" + value + "'"
